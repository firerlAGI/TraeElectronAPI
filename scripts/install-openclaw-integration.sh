...
if [[ -f "$quickstart_command" ]]; then
  ...
  quickstart_command_config="$(quote_shell_path "$quickstart_command")"
else
  ...
  quickstart_command_config="$(quote_shell_path "$node_binary") $(quote_shell_path "$resolved_repo_root/scripts/quickstart.js")"
fi
...
if [[ "$agent_count" =~ ^[0-9]+$ ]] && [[ "$agent_count" -gt 0 ]]; then
  for ((i = 0; i < agent_count; i += 1)); do
    mode="$(update_tool_policy "agents.list[$i].tools.allow" "agents.list[$i].tools.alsoAllow" '["trae_status","trae_update_self","trae_new_chat","trae_open_project","trae_switch_mode","trae_delegate"]')"
